...
function ReciboSaidaDoacao({
  ...
  const gerarPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Recibo de Saída de Doação', 105, 20, { align: 'center' });
    doc.setFontSize(12);
    let y = 40;
    ...
    y += 30;
    doc.text('Assinatura do responsável pelo recebimento:', 10, y);
    y += 20;
    ...
    // Apenas uma linha para assinatura, sem underline extra
    y += 10;
    doc.text('Assinatura', assinaturaInicio + 50, y);
    // doc.save('recibo_saida_doacao.pdf');
    // Abrir em nova guia
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{fontSize: '1.2rem', color: '#388e3c', marginBottom: 18, textAlign: 'center', fontWeight: 600}}>Recibo de Saída de Doação</div>
        ...
        <div style={{margin: '32px 0 18px 0', textAlign: 'center'}}>
          ...
          <input ref={assinaturaRef} type="text" placeholder="Assinatura" style={{width: '100%', border: 'none', borderBottom: '2px solid #7fc98f', fontSize: '1.1rem', padding: '8px 0', textAlign: 'center', outline: 'none'}} />
        </div>
